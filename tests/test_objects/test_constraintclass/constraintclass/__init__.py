...
class ConstraintClass:
    ...
    def internalProperties(self):#internal
        self. constraintNum           = []                                            # Constraint number
    ...
    def checkLoc(self,action):
        ...
        if action == 'W':
            if self.loc == 999: # Because "Allow library block to modify its content" is selected in block's mask initialization, this command runs twice, but warnings cannot be displayed during the first initialization. 
                ...
                self.loc = [0, 0, 0]
                warnings.warn('For ', self.name, ': constraint.loc was changed from [9999, 9999, 9999] to [0, 0, 0]')
        elif action == 'E':
            if self.loc == 999:
                warnings.warn('For ', self.name, ': constraint.loc needs to be specified in the WEC-Sim input file. constraint.loc is the [x y z] location, in meters, for the pitch constraint.')
