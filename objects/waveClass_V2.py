        ...
        if rampTime == 0:
            for i in range(maxIt+1):
                for idir in range(np.size(self.waveDir)):
                    ...
                    self.waveAmpTime[1][i]    = self.waveAmpTime[1][i] + sum(tmp1)
                    self.waveAmpTime1[1][i]   = self.waveAmpTime1[1][i] + sum(tmp11)
                    self.waveAmpTime2[1][i]   = self.waveAmpTime2[1][i] + sum(tmp12)
                    self.waveAmpTime3[1][i]   = self.waveAmpTime3[1][i] + sum(tmp13)
        else:
            for i in range(maxRampIT):
                for idir in range(np.size(self.waveDir)):
                    ...
                    tmp12   = tmp*np.real(np.exp(((-1)**0.5)*(self.wF*t - self.wk*(self.wavegauge2loc[0]*np.cos(self.waveDir[idir]*np.pi/180) + self.wavegauge2loc[1]*np.sin(self.waveDir[idir]*np.pi/180)) + self.phase[idir])))
                    tmp13   = tmp*np.real(np.exp(((-1)**0.5)*(self.wF*t - self.wk*(self.wavegauge3loc[0]*np.cos(self.waveDir[idir]*np.pi/180) + self.wavegauge3loc[1]*np.sin(self.waveDir[idir]*np.pi/180)) + self.phase[idir])))
                    self.waveAmpTime[1][i]    = self.waveAmpTime[1][i] + sum(tmp1)*(1+np.cos(np.pi+np.pi*(i)/maxRampIT))/2
                    self.waveAmpTime1[1][i]   = self.waveAmpTime1[1][i] + sum(tmp11)*(1+np.cos(np.pi+np.pi*(i)/maxRampIT))/2
                    self.waveAmpTime2[1][i]   = self.waveAmpTime2[1][i] + sum(tmp12)*(1+np.cos(np.pi+np.pi*(i)/maxRampIT))/2
                    self.waveAmpTime3[1][i]   = self.waveAmpTime3[1][i] + sum(tmp13)*(1+np.cos(np.pi+np.pi*(i)/maxRampIT))/2
    ...
    def waveElevUser(self,rampTime,dt,maxIt,data,t):
        ...
        if rampTime != 0:
            for i in range(maxRampIT+1):
                ...
                self.waveAmpTime[1][i] = self.waveAmpTime[1][i]*(1+np.cos(np.pi+np.pi*(i)/maxRampIT))/2
