...
class BodyClass:
    ...
    def readH5file(self):
        ...
        if np.array(f.get(name + '/properties/mass')).all() != None:
            ...
            g_m = {'mass' : [tmp[0][self.dof_start+5:self.dof_end+1],tmp[1][self.dof_start+5:self.dof_end+1]]}
        else:
            g_m = {}
        if np.array(f.get(name + '/properties/stiffness')).all() != None:
            tmp = np.array(f.get(name + '/properties/stiffness'))
            g_s = {'stiffness' : [tmp[0][self.dof_start+5:self.dof_end+1],tmp[1][self.dof_start+5:self.dof_end+1]]}
        else:
            g_s = {}
        if np.array(f.get(name + '/properties/damping')).all() != None:
            tmp = np.array(f.get(name + '/properties/damping'))
            g_d = {'damping' : [tmp[0][self.dof_start+5:self.dof_end+1],tmp[1][self.dof_start+5:self.dof_end+1]]}
        else:
            g_d = {}
        gbm = {**g_m,**g_s,**g_d}
    ...
    def hydroForcePre(self,w,waveDir,CIkt,CTTime,numFreq,dt,rho,g,waveType,waveAmpTime,iBod,numBod,ssCalc,nlHydro,B2B):
        ...
        if self.dof_gbm > 0:
            ...
            self.linearDamping(1:tmp1(1),1:tmp1(2)) = tmp0 

            tmp0 = self.viscDrag.Drag
            tmp1 = size(self.viscDrag.Drag)
            self.viscDrag.Drag = zeros (self.dof)                
            self.viscDrag.Drag(1:tmp1(1),1:tmp1(2)) = tmp0 
        ...
        self.hydroForce.userDefinedFe = zeros(length(waveAmpTime(:,2)),self.dof)   #initializing userDefinedFe for non imported wave cases
        switch waveType
            case {'noWave'}
                self.noExcitation()
        ...
        if (gbmDOF>0)
            ...
            self.hydroForce.gbm.mass_ff=self.hydroForce.fAddedMass(7:self.dof,self.dof_start+6:self.dof_end)+self.hydroData.gbm.mass   # need scaling for hydro part
            self.hydroForce.fAddedMass(7:self.dof,self.dof_start+6:self.dof_end) = 0
            self.hydroForce.gbm.mass_ff_inv=inv(self.hydroForce.gbm.mass_ff)
            
            # state-space formulation for solving the GBM
            self.hydroForce.gbm.state_space.A = [zeros(gbmDOF,gbmDOF),...
                eye(gbmDOF,gbmDOF)...  # move to ... hydroForce sector with scaling .
                -inv(self.hydroForce.gbm.mass_ff)*self.hydroForce.gbm.stiffness,-inv(self.hydroForce.gbm.mass_ff)*self.hydroForce.gbm.damping]             # or create a new fun for all flex parameters
            self.hydroForce.gbm.state_space.B = eye(2*gbmDOF,2*gbmDOF)
            self.hydroForce.gbm.state_space.C = eye(2*gbmDOF,2*gbmDOF)
            self.hydroForce.gbm.state_space.D = zeros(2*gbmDOF,2*gbmDOF)
